...
function Header({title, isBackButton, showCart}) {
  ...
  return (
    <View style={styles.container}>
      {isBackButton && (
        <Pressable onPress={handleBackClick} style={styles.pressableLeftView}>
      ...
      {showCart && (
        <Pressable onPress={handleCartClick} style={styles.pressableRightView}>
          <SVGCart width={24} height={24} />
          {cartData && cartData.length > 0 && (
            <View style={styles.badgeView}></View>
          )}
        </Pressable>
      )}
    </View>
